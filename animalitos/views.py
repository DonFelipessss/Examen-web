...
from .models import Subseccion, Producto

class TiendaView(View):
    def get(self, request):
        subsecciones = Subseccion.objects.all()
        context = {'subsecciones': subsecciones}
        return render(request, 'animalitos/tienda.html', context)

class SubseccionView(View):
    def get(self, request, subseccion_id):
        subseccion = get_object_or_404(Subseccion, id=subseccion_id)
        productos = subseccion.producto_set.all()
        context = {'subseccion': subseccion, 'productos': productos}
        return render(request, 'animalitos/subseccion.html', context)

class DetalleProductoView(View):
    def get(self, request, producto_id):
        producto = get_object_or_404(Producto, id=producto_id)
        context = {'producto': producto}
        return render(request, 'animalitos/detalle_producto.html', context)
...
class DonacionesView(TemplateView):
    template_name = 'donaciones.html'

class Nosotros(View):
    template_name = 'accounts/nosotros.html'
...
class LogInView(GuestOnlyView, FormView):
    template_name = 'animalitos/base.html'
...
class SignUpView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        ...
        if settings.ENABLE_USER_ACTIVATION:
            ...
            messages.success(request, _('¡te has conectado exitosamente!'))

            return redirect(reverse('animalitos:base'))
